...
| 自动化门禁 | changed-and-verified | 阶段31发布候选 135 tests、0 failed、0 skipped；compileall、node check、diff check、OpenAPI、canonical API 与 CI 配置均已复核。 |
...
GitHub `main` 是 V1 开发基线。阶段31已审计并完成新仓库主线合并；后续阶段仍必须在独立 `codex/stage-<编号>-<主题>` 分支完成提交和推送，仍待审计或质量复核的变更以 Draft PR 交接。
