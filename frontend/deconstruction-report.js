...
(() => {
  ...
  function render(report) {
    return `<section class="analysis-report"><header class="analysis-intro"><span class="eyebrow">小说细读 · ${esc(report.producer)}</span><h2>${esc(report.title)}</h2><p>${esc(report.scope)}</p><div class="analysis-counts"><span>${report.characters.length} 张人物卡</span><span>${report.events.length} 个剧情节点</span><span>${report.evidence.length} 处原文依据</span></div></header><div class="analysis-findings">${report.findings.map((f, i) => `<details ><summary><span class="analysis-number">0${i + 1}</span>${esc(f.title)}<span class="analysis-badge">${labels[f.status]}</span></summary><p>${esc(f.text)}</p><div data-finding-evidence="${esc(f.id)}"></div></details>`).join("")}</div><nav class="analysis-tabs" aria-label="拆解视角"><button type="button" data-report-tab="plot" aria-pressed="true">剧情地图</button><button type="button" data-report-tab="characters" aria-pressed="false">人物卡</button><button type="button" data-report-tab="time" aria-pressed="false">双时间线</button></nav><div class="analysis-view"></div><details class="analysis-questions"><summary>读到这里，还不能确定的事 · ${report.open_questions.length}</summary><ul>${report.open_questions.map(q => `<li>${esc(q)}</li>`).join("")}</ul></details><p class="analysis-footnote">本轮为已读章节的局部分析。引用位置已匹配正文，文学解释仍可继续讨论和修正。</p></section>`;
  }
    ...
    function draw() {
      ...
      if (tab === "characters") {
        view.innerHTML = `<div class="analysis-characters">${report.characters.map(p => {
          ...
          return `<article class="analysis-person"><header><span class="eyebrow">${esc(p.role)}</span><h3>${esc(p.name)}</h3></header><p class="analysis-portrait">${esc(portrait.text)}</p><div class="analysis-experience"><h4>关键经历</h4><p>${esc(p.change.text)}</p></div><details class="analysis-person-more"><summary>展开依据与人物细节</summary><h4>为人判断的依据</h4>${claim(portrait)}<h4>经历的依据</h4>${evidence(p.change.evidence_ids)}<h4>身份与动机</h4>${claim(p.identity)}${claim(p.motivation)}</details></article>`;
        }).join("")}</div>`;
      } else if (tab === "time") {
